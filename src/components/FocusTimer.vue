    ...
    <div class="text-center space-y-6">
      ...
      <div class="relative inline-block">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <div v-else-if="isPaused" class="text-xs text-orange-500 mt-1">
            Paused
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-center gap-3">
        ...
        <template v-else>
          ...
          <button
            @click="resume"
            v-else
            class="px-6 py-3 bg-green-500 text-white rounded-xl transition-all font-medium flex items-center gap-2 rounded-full"
          >
            <svg
              class="w-9 h-9"
              fill="none"
          ...
          <button
            @click="reset"
            class="px-6 py-3 bg-gray-500 text-white rounded-xl hover:bg-gray-600 transition-all font-medium flex items-center gap-2 rounded-full"
          >
            <svg
              class="w-9 h-9"
              fill="none"
...
// Global singleton ticker so the timer keeps running across route changes
...
const totalDuration = computed(() => {
  return timerType.value === "focus"
    ? focusStore.focusDuration
    : focusStore.breakDuration;
});
...
onMounted(() => {
  ...
  // Ensure a single global ticker continues running when this component unmounts
  if (!win.__focusTicker) {
    win.__focusTicker = setInterval(() => {
      focusStore.tick();
    }, 1000);
  }

  // Clean up on full page unload only
  const beforeUnload = () => {
    if (win.__focusTicker) {
      clearInterval(win.__focusTicker);
      win.__focusTicker = null;
    }
  };
...
onUnmounted(() => {
  // Do not clear the global ticker here so it survives route changes.
  // Any per-instance resources can be cleaned here if added later.
});
</script>
